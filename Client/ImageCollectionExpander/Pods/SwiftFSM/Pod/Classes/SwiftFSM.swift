  ...
  */
  public func addState(newState: State) -> SwiftFSMState<State, Transition> {
    if let existingState = _states[newState] {
      log(message: "State \(newState) already added")
  ...
  public func startFrom(state: State) {
    if _currentState == nil {
      if let fsmState = _states[state] {
        ...
        log(message: "Cannot find the \(state) start state")
      }
    } else {
      log(message: "Fsm already started. Cannot startFrom again")
    }
  }
  ...
  */
  public func transitionWith(transition: Transition) -> State? {
    if let oldState = _currentState {
      if let newState = oldState.transitionWith(transition: transition) {
        if let newFsmState = _states[newState] {
          log(message: "\(oldState.state) -> \(transition) -> \(newState)")
          _currentState = newFsmState
          oldState.exit(withTransition: transition)
          newFsmState.enter(withTransition: transition)
          return newState
        } else {
          log(message: "The \(transition) transitions to an unregistered \(newState) state")
        }
      } else {
        log(message: "There is no transition defined from the [\(oldState.state)] state with the [\(transition)] transition")
      }
    } else {
      log(message: "Please start the fsm first")
    }
